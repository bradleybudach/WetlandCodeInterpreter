...
def code_parse(code):
    ...
    if code_check(code,goodCodeList,badCodeList,oldCodeList)==code:
        parsedDict = {
            'code': "Err: Not a valid code|"
            }
        return parsedDict
    else:
        if code in oldCodeList:
            print("Err: This code is no longer valid, but is a legacy code. When it was in use, this is the habitat the attribute described:|")
        if code == 'Pf':
            system = 'P'
            mod = 'f'
        if r'/' in code:
            ...
            if code.count(r'/') > 1:
                print("Err: too many splits|")
                return
            if len(code)<split_position+3:
                print("Err: need more characters after the split|")
                return
        system=code[0]
        if code== 'Pf':
...
if len(pd) == 1: # pd length will be 1 when code is invalid
    for key in pd:
        print((pd[key])[:-1]) # removes final

#print out info for valid code:
returnStr = "";
for key in pd:
        ...
        if key in ('System','Subsystem lookup','Class1','Class2'):
            if key in ('Subsystem lookup'):
                if pd['Subsystem'] == '':
                    ...
                    returnStr += 'Subsystem->' + ParsedCodeDict[pd[key]] + '|'
            elif key in ('Class1', 'Class2'):
                returnStr += key + '->' + ParsedCodeDict[pd[key]] + '|'
            else:
                returnStr += 'System->' + ParsedCodeDict[pd[key]] + '|'
        elif key in ('Subclass1 lookup','Subclass2 lookup','Waterregime'):
            if key == 'Subclass1 lookup':
                if pd['Subclass1'] == '':
                    pass
                else:
                    returnStr += 'Subclass1->' + ParsedCodeDict2[pd[key]] + '|'
            elif key == 'Subclass2 lookup':
                returnStr += 'Subclass2->' + ParsedCodeDict2[pd[key]] + '|'
            else:
                returnStr += 'Water Regime->' + ParsedCodeDict2[pd[key]] + '|'
        elif key in ('Modifiers'):
            for modifier in pd[key]:
                #print(modifier)
                returnStr += 'Modifier->' + ParsedCodeDict2[modifier] + '|'

print(returnStr[:-1]) # removes last '|' character and prints to output stream